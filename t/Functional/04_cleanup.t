...
my @types =
  ( 'VirtualMachine', 'ResourcePool', 'Folder', 'DistributedVirtualSwitch' );
...
for my $type (qw(ResourcePool Folder DistributedVirtualSwitch)) {
    ...
    ok( \&VCenter::destroy_entity( 'test_1337', $type ),
        "Destroying test_1337 $type" );
    is(
        ...
        undef,
        "test_1337 $type doesn't exist after delete"
    );
}
...
for my $type (@types) {
    is(
        ...
        undef,
        "test_1337 $type doesn't exist after cleanup"
    );
}
diag("Creating resources with entity and see if they don't get deleted");
&Test::create_test_entities;
ok( \&VCenter::create_dvportgroup( 'test_1337_dvg', 'test_1337' ),
    "Creating test_1337_dvg DVPG" );
throws_ok { &VCenter::create_dvportgroup( 'test_1337_dvg', 'test_1337' ) }
'Entity::NumException', 'Exception is thrown by second dvg creation';
throws_ok {
    &VCenter::create_dvportgroup( 'test_1337_dvg2', 'test_1337_test_1337' );
}
'Entity::NumException', 'Exception is thrown if no parent switch found';
&Test::create_test_vm('test_1337');
&admin::cleanup;
for my $type (@types) {
    is( &VCenter::exists_entity( 'test_1337', $type ),
        1, "Entity Exists $type" );
}
...
$view =
  &Guest::entity_name_view( 'test_1337_dvg', 'DistributedVirtualPortgroup' );
