...
BEGIN {
    &Opts::add_options(
        %{
            $entity::module_opts->{functions}->{add}->{functions}->{snapshot}
              ->{opts}
        }
    );
    &Opts::parse();
...
throws_ok { &Guest::list_snapshot( $view->name ) } 'Entity::Snapshot',
  "list snapshot returns Exception when no snapshot present";
is( &Guest::create_snapshot( $view->name, 'test', 'test' ),
    1, "Create 1st snapshot was succesful" );
$view->update_view_data();
output_like(
    sub {
        &Guest::traverse_snapshot(
            $view->snapshot->rootSnapshotList->[0],
            $view->snapshot->currentSnapshot->value
        );
    },
qr/^\*CUR\*\sID\s=>\s'1',\sname\s=>\s'test',\screateTime\s=>\s'[^']*',\sdescription\s=>\s'test'$/,
    qr/^$/,
    "traverse snapshot returns one snapshot"
);
output_like(
    \&entity::list_snapshot,
qr/^\*CUR\*\sID\s=>\s'1',\sname\s=>\s'test',\screateTime\s=>\s'[^']*',\sdescription\s=>\s'test'$/,
    qr/^$/,
    "List snapshot output is correct on stdout and stderr"
);
is(
    &Guest::traverse_snapshot(
        $view->snapshot->rootSnapshotList->[0],
    ...
    1,
    "Traverse snapshot returned 1"
);
is( &entity::add_snapshot(), 1, "Create 2nd snapshot was succesful" );
output_like(
    \&entity::list_snapshot,
qr/^ID\s=>\s'1',\sname\s=>\s'test',\screateTime\s=>\s'[^']*',\sdescription\s=>\s'test'\n\*CUR\*/,
    qr/^$/,
    "List snapshot output is correct on stdout and stderr"
);
&Opts::add_options(
    %{
        $entity::module_opts->{functions}->{change}->{functions}->{snapshot}
          ->{opts}
    }
);
...
&Opts::add_options(
    %{
        $entity::module_opts->{functions}->{delete}->{functions}->{snapshot}
          ->{opts}
    }
);
...
throws_ok { &entity::list_snapshot() } 'Entity::Snapshot',
  "list_snapshot throws exception when no snapshot is present";
is( &entity::add_snapshot(), 1, "Create 3nd snapshot was succesful" );
output_like(
    \&entity::list_snapshot,
qr/^\*CUR\*\sID\s=>\s'3',\sname\s=>\s'[^']*',\screateTime\s=>\s'[^']*',\sdescription\s=>\s'[^']*'$/,
    qr/^$/,
    "List snapshot output is correct on stdout and stderr"
);
