...
SKIP: {
    ...
    skip "TEMPLATE_HASH env variable not seen", 1, if $@;
    ...
    for my $os_temp ( @{ &Support::get_keys("template") } ) {
            ...
            && defined( $os_temp_view->snapshot->rootSnapshotList ) )
        {
            ...
            $snapshot_view =
              &VCenter::moref2view( $snapshot_view->[0]->{'snapshot'} );
        }
        else {
        ...
        isa_ok( &Support::identification_domain, 'CustomizationIdentification',
            "identification_domain returned CustomizationIdentification object"
        );
        isa_ok(
            &Support::identification_workgroup,
...
"identification_workgroup returned CustomizationIdentification object"
        ...
        my @adapters = @{
            &Guest::CustomizationAdapterMapping_generator(
                $os_temp_view->name
            )
        };
        ...
        for my $interface (@interfaces) {
            isa_ok( $interface, 'HASH',
                "Network interface returned is a hash" );
        }
    }
}
SKIP: {
