...
=item delete
    ...
    Delete all entities attsached to a ticket
...
=head1 FILES
    ...
    VI_USERNAME=
    VI_PASSWORD=
    VI_SERVER=vcenter.ittest.balabit
    VI_URL=https://vcenter.ittest.balabit/sdk
    VI_PORTNUMBER=443
    VI_PROTOCOL=HTTPS
...
=item TEST_PERL_MODULES.pl
    ...
    If you would like to test if the modules for running the test framework are installed run it with env variable TEST
...
=head3 test_variables
    ...
    Some tests are skipped because normal environment they shouldnt be run or used. If required to be run the following environemtn variables need to be configured:

=item DNS

    This will run DNS test to see if resolve is working

=item AUTHOR

    This will check if AUTHOR has not forgotten to update specific tests, or left FIXME -s in the code
