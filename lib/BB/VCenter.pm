...
sub linked_clone_folder {
    my ( $temp_name ) = @_;
    &Log::debug("Starting Vcenter::linked_clone_folder sub, temp_name=>'$temp_name'");
    my $temp_fol;
...
sub create_resource_pool {
    ...
    &Log::debug("Starting VCenter::create_resource_pool sub, rp_name=>'$rp_name', rp_parent=>'$rp_parent'");
    my $type = 'ResourcePool';
    if ( &exists_entity( $rp_name, $type ) ) {
        &Log::debug("Resource pool already exists on Vcenter");
        Entity::NumException->throw( error => 'Resource pool already exists. Cannot create', entity => $rp_name, count => '1' );
    } elsif ( !&exists_entity( $rp_parent, $type ) ) {
        &Log::debug("Resource pool parent doesn't exists on Vcenter");
        Entity::NumException->throw( error => 'Resource pool parent doesn\'t exist. Cannot create', entity => $rp_parent, count => '0' );
    }
    my $rp_parent_view = Vim::find_entity_view( view_type => $type, properties => [ 'name' ], filter => { name => $rp_parent } );
    ## Creation objects
    ...
    my $rp_name_view = $rp_parent_view->CreateResourcePool( name => $rp_name, spec => $rp_spec );
    if( $rp_name_view->type ne $type ) {
        Entity::NumException->throw( error => 'Could not create resource pool', entity => $rp_name, count => '0' );
    }
...
sub create_folder {
    ...
    &Log::debug("Starting VCenter::create_folder sub, fol_name=>'$fol_name', fol_parent=>'$fol_parent'");
    my $type = 'Folder';
    if ( &exists_entity( $fol_name, $type ) ) {
        &Log::debug("Folder already exists on Vcenter");
        Entity::NumException->throw( error => 'Folder already exists. Cannot create', entity => $fol_name, count => '1' );
    } elsif ( !&exists_entity( $fol_parent, $type ) ) {
        &Log::debug("Folder parent doesn't exists on Vcenter");
        Entity::NumException->throw( error => 'Folder parent doesn\'t exist. Cannot create', entity => $fol_parent, count => '0' );
    }
    ...
    if( $fol_name_view->type ne $type ) {
        Entity::NumException->throw( error => 'Could not create folder', entity => $fol_name, count => '0' );
    }
...
sub service_content {
    ...
    if ( !defined( $sc ) ) {
        Vcenter::ServiceContent->throw( error => 'Could not retrieve service content' );
    }
    return $sc;
}
1
